...
async def get_current_user(
    db: AsyncSession = Depends(get_db), token: str = Depends(outh2_scheme)
) -> User:
    """获取当前认证用户"""
    import logging
...
async def get_current_hr_user(current_user: User = Depends(get_current_user)) -> User:
    ...
    if (
        current_user.role not in [UserRole.HR_MANAGER, UserRole.HR_SPECIALIST]
        and not current_user.is_superuser
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="HR permissions required"
        )
    return current_user
...
async def get_current_admin_by_role(
    ...
    if "超级管理员" not in roles:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="需要超级管理员角色"
        )
    return current_user
...
def require_and_role_names(required: List[str]):
    async def dependency(
        ...
        query = (
            ...
            .join(UserRoleAssociation, Role.id == UserRoleAssociation.role_id)
            .where(
                UserRoleAssociation.user_id == current_user.id, Role.is_active == True
            )
        )
        result = await db.execute(query)
        roles = {r.name for r in result.scalars().all()}
        if not any(name in roles for name in required):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="角色权限不足"
            )
        return current_user
