...
class JobDescriptionService:
    ...
    async def create_job_description(
        self, jd_data: JobDescriptionCreate, user_id: UUID
    ) -> JobDescriptionResponse:
        """
        创建新的职位描述
        ...
        try:
            ...
            result_data = await remote_service_client.post(
                endpoint="/job-descriptions/save", data=request_data, user_id=user_id
            )

            jd_response = self._build_job_description_response(
                result_data, request_data, user_id
            )
    ...
    def _build_job_description_response(
        ...
        merged_data.setdefault(
            "workflow_type", request_data.get("workflow_type") or "jd_generation"
        )
    ...
    async def update_job_description(
        self, jd_id: str, jd_data: JobDescriptionUpdate, user_id: UUID
    ) -> JobDescriptionResponse:
        """
        更新职位描述
        ...
        try:
            ...
            return self._build_job_description_response(
                result_data, request_data, user_id, jd_id=jd_id
            )
    ...
    async def get_job_description(
        ...
        try:
            ...
            result_data = await remote_service_client.get(
                endpoint=f"/job-descriptions/{jd_id}", user_id=user_id
            )
    ...
    async def list_job_descriptions(
        ...
        """
        logger.info(
            f"📥 获取JD列表请求: page={page}, size={size}, status_filter={status_filter}, user_id={user_id}"
        )

        try:
            # 准备查询参数
            additional_params = {"page": page, "size": size}
            if status_filter:
    ...
    async def delete_job_description(self, jd_id: str, user_id: UUID) -> Dict[str, Any]:
        """
        删除职位描述（软删除）
        ...
        try:
            ...
            try:
                ...
                for criteria in criteria_list.items:
                    try:
                        await scoring_service.delete_scoring_criteria(
                            str(criteria.id), user_id
                        )
                        deleted_criteria.append(
                            {"id": str(criteria.id), "title": criteria.title}
                        )
                        ...
                        logger.warning(
                            "删除 JD 关联评分标准失败 jd_id=%s criteria_id=%s: %s",
                            jd_id,
                            criteria.id,
                            exc,
                        )
            ...
            logger.info(
                f"成功删除职位描述: {jd_id}，关联删除评分标准 {len(deleted_criteria)} 条"
            )
            return result_data
